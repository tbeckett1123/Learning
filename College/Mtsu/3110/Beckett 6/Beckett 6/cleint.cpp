...
template <typename R>
void Read(const string&, RBT<R>&);
void Rent(RBT<Video>&, RBT<Customer>&);
void Return(RBT<Video>&);
void Show(RBT<Video>&);
void Members(RBT<Customer>&);
...
int main()
{
	...
	do
	{
		...
		default:  vList.Write("movies2.dat");	
				  cout<<"Thank You\n";
		};	
	}while(choice != 'f');
...
template <typename R>
void Read(const string& file, RBT<R>& list)
...
void Rent(RBT<Video>& vList, RBT<Customer>& cList)
...
void Members(RBT<Customer>& cList)
